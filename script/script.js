...
function simulate() {
  ...
  if (level === 'warning') {
    pushAlert('warning', `Peringatan! Skor ${score}/100. Potensi bahaya signifikan.`)
    sendAlertNotification(mag, "Pesisir Cilacap")
  } else if (level === 'elevated' && tick % 5 === 0) {
    pushAlert('elevated', `Waspada. Skor ${score}/100. Tren meningkat.`)
  }
...
function renderStatus() {
  ...
  // Atmospheric border pulse transition
  if (state.level === 'warning') {
    mainHeader.classList.add('atmospheric-alert')
    try { siren.play() } catch (e) {}
  } else {
    mainHeader.classList.remove('atmospheric-alert')
  }
}
...
function pushAlert(level, msg) {
  ...
  wrap.innerHTML = `<div class="alert-header">
                      <span class="alert-title">${level === 'warning' ? 'PERINGATAN' : 'WASPADA'}</span>
                      <small>${when} WIB</small>
                    </div>
                    <div style="flex:1; margin-top: 4px;">${msg}</div>`
...
function updateQuakeDetails() {
  ...
  quakeBox.innerHTML = `
    <p><strong>Waktu:</strong> <span>${q.time} WIB</span></p>
    <p><strong>Magnitudo:</strong> <span>${q.mag}</span></p>
    <p><strong>Kedalaman:</strong> <span>${q.depth} km</span></p>
    <p><strong>Tide Gauge:</strong> <span>${q.tide} m</span></p>
    <p><strong>Wave/Buoy:</strong> <span>${q.wave} m</span></p>
    <p><strong>Skor Risiko:</strong> <span>${q.score}/100</span></p>
  `
